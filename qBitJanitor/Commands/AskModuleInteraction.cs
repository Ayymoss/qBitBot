    ...
    : InteractionModuleBase<SocketInteractionContext>
{
    ...
    [DefaultMemberPermissions(GuildPermission.SendMessages)]
    [MessageCommand("Answer Question")]
    public async Task AnswerQuestionAsync(IMessage message)
    {
        if (message.Author is not SocketGuildUser guildUser) return;
        ...
        try
        {
            ...
            await messageProcessingService.AnswerQuestionAsync(guildUser.Id, string.Join(" ", relatedMessages), async (success, response)
                => await FollowupAsync(response));
        }
        catch (Exception e)
        {
            await FollowupAsync("Error during response...", ephemeral: true);
            logger.LogError(e, "Error during AnswerQuestionAsync execution. Author {Author}, Question {Question}", message.Author.Username,
                message.Content);
        }
    }
}
